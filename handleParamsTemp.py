...
from configParams import shortCutDateIDs, shortCutName, apiDict, paramTempVerNameArr
...
class TestParamsTemp(object):
    ...
    def test(self):
        ...
        # 复制参数组模板 | v6.5.x_default 
        targetVersion = paramTempVerNameArr[0] 
        targetTempTextEle = webWaitEle(
            self, (By.XPATH, "//*[contains(text(), '{}')]".format(targetVersion)))
       
        targetTempEle = targetTempTextEle.find_element(
            By.XPATH, '../..')
        print('--------->', targetTempEle.get_attribute('innerHTML'))  
        copyParamTempBtn = targetTempEle.find_element(
            By.NAME, 'copyParamTempBtn')
        copyParamTempBtn.click()
