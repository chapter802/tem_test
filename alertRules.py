...
from configParams import alertLevels, alertTypes, opArr, alertFrequencyUnits

randomStr = util.get_random_string(6)

apiKeyArr = ['queryAlertRuleList', 'createAlertRule',
             'updateAlertRule', 'deleteAlertRule', 'queryAlertRuleIndicators']
...
class TestAlertRules(object):
    ...
    def test(self):
        ...
        for x in alertLevels:
            ...
            util.getRequsetInfo(
                self, self.driver.requests, 'ajax/api/v1/observe/alert/rules',  '查询告警规则', 'POST')
            sleep(2)
        ...
        sleep(1)
        util.getRequsetInfo(
            self, self.driver.requests, 'ajax/api/v1/observe/alert/rule', '创建告警规则', 'POST', 'rules')
        sleep(2)
        ...
        def closeModal(self):
            ...
            util.getRequsetInfo(
                self, self.driver.requests, 'ajax/api/v1/observe/alert/rules/',  '编辑告警规则', 'PUT', '', closeModal)
        ...
        if len(statusBtns) > 0:
            ...
            sleep(1)
            util.getRequsetInfo(
                self, self.driver.requests, 'ajax/api/v1/observe/alert/rules/',  '切换告警规则状态', 'PUT')
            sleep(3)
            statusBtns[0].click()
            sleep(1)
            self.driver.find_element(
        ...
        if len(deleteBtns) > 0:
            ...
            sleep(1)
            util.getRequsetInfo(
                self, self.driver.requests, 'ajax/api/v1/observe/alert/rules/',  '删除告警规则', 'DELETE')
        # util.getAllReqInfo(
        #     self, self.driver.requests, apiKeyArr, closeModal)
        sleep(5)
        self.driver.quit()
